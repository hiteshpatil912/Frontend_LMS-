<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-950">Lessons</h1>
        <p class="text-sm text-slate-500">Manage all course lessons across the platform.</p>
      </div>
    ...
    <div v-else-if="!store.items.length" class="flex flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 py-20 text-center">
      <p class="text-slate-500">No lessons found.</p>
    </div>
...
const load = () => store.fetchData('lessons')
