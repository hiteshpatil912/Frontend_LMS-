<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-950">System Reports</h1>
        <p class="text-sm text-slate-500">Access platform analytics and data exports.</p>
      </div>
    ...
    <div v-else-if="!store.items.length" class="flex flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 py-20 text-center">
      <p class="text-slate-500">No reports generated yet.</p>
    </div>
        ...
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-slate-200">
            <tr v-for="item in store.items" :key="item.id" class="hover:bg-slate-50">
              <td class="px-6 py-4 font-medium text-slate-900">{{ item.name || item.title }}</td>
              <td class="px-6 py-4">
                <span class="inline-flex rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 uppercase">
                  {{ item.type || 'PDF' }}
                </span>
              </td>
              ...
              <td class="px-6 py-4 text-right">
                <button class="text-brand-600 hover:text-brand-700 font-medium">Download</button>
              </td>
            </tr>
          </tbody>
        </table>
...
const load = () => store.fetchData('reports')
