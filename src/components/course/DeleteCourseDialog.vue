          ...
          <TransitionChild as="template" enter="duration-200 ease-out" enter-from="scale-95 opacity-0" enter-to="scale-100 opacity-100" leave="duration-150 ease-in" leave-from="scale-100 opacity-100" leave-to="scale-95 opacity-0">
            <DialogPanel class="w-full max-w-md rounded-lg bg-white p-6 shadow-soft">
              <div class="flex gap-4">
                ...
                <div>
                  <DialogTitle class="text-lg font-semibold text-slate-950">Delete course</DialogTitle>
                  <DialogDescription class="mt-2 text-sm text-slate-500">
                    This will permanently delete <span class="font-medium text-slate-700">{{ course?.title }}</span> and remove it from course management.
                  </DialogDescription>
                </div>
              </div>
              <div class="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <button class="focus-ring rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50" type="button" @click="$emit('close')">Cancel</button>
                <button class="focus-ring inline-flex justify-center rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-70" type="button" :disabled="deleting" @click="$emit('confirm')">
